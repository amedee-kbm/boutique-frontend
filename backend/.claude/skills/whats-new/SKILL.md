---
name: whats-new
description: Draft a "What's New" community update for the the upstream project Discord covering changes across backend, frontend, and infra over a time window. Use when the user runs /whats-new or asks for a community changelog/release-roundup post for organizers (and the programmers in the room).
allowed-tools: Bash(git:*), Bash(gh:*), Bash(date:*), Bash(grep:*), Bash(ls:*), Bash(test:*), Read, Edit, Write, Glob, Grep
---

# What's New

Produces a Discord-ready "What's New" post for the the upstream project community and archives it on the
...
Three repos, all **public**, all siblings of the cwd (`upstream-backend`):

| Repo | Path | Primary source |
|------|------|----------------|
| Backend | `.` (cwd) | `CHANGELOG.md` (rich, dated, user-facing prose) |
| Frontend | `../upstream-frontend` | `CHANGELOG.md` **if present**, else git/PR digging |
| Infra | `../infra` | git/PR digging (no changelog) |

> The frontend is adopting the backend's changelog process. **Always check for
> `../upstream-frontend/CHANGELOG.md` first** and use it changelog-first when it exists; only fall
...
```bash
test -f ../upstream-frontend/CHANGELOG.md && echo "use changelog" || echo "dig git/PRs"
# changelog path: same as backend, against ../upstream-frontend/CHANGELOG.md
# fallback path:
git -C ../upstream-frontend log --since="<cutoff>" --first-parent --merges --pretty='%h %s'
gh -R the upstream frontend repository pr list --state merged --search "merged:>=<cutoff>" \
...
gh -R the upstream infrastructure repository pr list --state merged --search "merged:>=<cutoff>" \
   --limit 50 --json number,title,body,mergedAt
```
...
```markdown
**📋 Full changelogs:** [Backend](https://github.com/the upstream backend repository/blob/main/CHANGELOG.md) · [Frontend](https://github.com/the upstream frontend repository/blob/main/CHANGELOG.md)
```

Only list Frontend once `../upstream-frontend/CHANGELOG.md` exists; add Infra if/when it gets one.
...
```
📖 Full update → https://the upstream docs site/whats-new/<slug>/
```
...
- The docs publish to https://the upstream docs site under the "What's New" nav tab; each edition lives
  at `https://the upstream docs site/whats-new/<slug>/`.
