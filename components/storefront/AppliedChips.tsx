...
import { bucketById } from '@/lib/storefront/filters'
      ...
      {state.f.map((id) => {
        ...
        return (
          <Chip
            key={id}
            label={label}
            onRemove={() => setState({ f: state.f.filter((v) => v !== id) })}
          />
        )
      })}

      {state.colour.map((hex) => (
        <button
          key={hex}
          type="button"
          onClick={() => setState({ colour: state.colour.filter((v) => v !== hex) })}
